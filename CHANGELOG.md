...
- `advisories_available` label on `nextcloud_system_update_available`, telling whether the
  GitHub security advisories could be used. While it is `false` any available update is
  reported.
- `NextcloudSecurityCheckUnavailable` alert in `contrib/prometheus-alerts.yaml`, firing
  when the advisories have been unusable for six hours.

### Changed

- `nextcloud_system_update_available` now only reports updates which contain security
  fixes, based on the advisories published for `nextcloud/security-advisories`. The
  identifiers of the vulnerabilities an update fixes are written to the log. Needs
  outgoing access to `api.github.com`; when the advisories are unreachable the metric
  falls back to reporting any available update.
- CI is now a single `ci.yml` sharing one reusable image workflow with `release.yml`.
  The image build no longer sets up QEMU, which was never used because the Dockerfile
  cross-compiles, and pull requests build only the host architecture.

### Removed

- Publishing of the Docker image to Docker Hub. Images are published to the GitHub
  Container Registry of the repository that builds them.
