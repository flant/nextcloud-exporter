...
var (
	...
	systemUpdateAvailableDesc = prometheus.NewDesc(
		...
		"Contains information whether a system update containing security fixes is available (0 = no, 1 = yes). The version label contains the installed nextcloud version, available_version the version nextcloud offers. The advisories_available label tells whether the GitHub security advisories could be used; while it is false any available update is reported. The fixed vulnerabilities are written to the log.",
		[]string{"version", "available_version", "advisories_available"}, nil)
...
func (c *nextcloudCollector) collectNextcloud(ch chan<- prometheus.Metric) error {
	...
	return readMetrics(ch, status, c.appsMetrics, c.updateMetrics, c.security)
}

func readMetrics(ch chan<- prometheus.Metric, status *serverinfo.ServerInfo, appsMetrics bool, updateMetrics bool, security *SecurityChecker) error {
...
func collectUpdate(ch chan<- prometheus.Metric, status *serverinfo.ServerInfo, security *SecurityChecker) error {
	...
	metric, err := prometheus.NewConstMetric(systemUpdateAvailableDesc, prometheus.GaugeValue, updateAvailableValue, systemInfo.Version, systemInfo.Update.AvailableVersion, advisoriesAvailable)
	if err != nil {
